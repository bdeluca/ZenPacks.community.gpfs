...
class IGPFSFileSystemMonitorInfo(IComponentInfo):
    filesystem_status = schema.TextLine(title=_t('GPFS FS State'))


class IGPFSStgPoolMonitorInfo(IComponentInfo):
    num_disks = schema.TextLine(title=_t('Num Disks'))
    

class IGPFSDiskMonitorInfo(IComponentInfo):
    gpfsDiskFSName = schema.TextLine(title=_t('fs name'))
    gpfsDiskStgPoolName = schema.TextLine(title=_t('stgpool name'))
    gpfsDiskMetadata = schema.TextLine(title=_t('has metadata'))
